...
const DATA_DIR = path.join('/tmp', 'cassport-data');
const UPLOAD_DIR = path.join('/tmp', 'cassport-uploads');
...
// CARRIERS
app.get('/api/carriers', (req, res) => { res.json(readData('carriers.json')); });
...
app.get('/api/dashboard/stats', (req, res) => {
  ...
  res.json({ totalSpend, billsProcessed: invoices.length, activeCarriers: carriers.length, pendingApprovals: pending });
});
app.get('/api/dashboard/spend-by-mode', (req, res) => {
  const invoices = readData('invoices.json');
  const modes = {};
  invoices.forEach(i => { const m = i.mode || 'Other'; modes[m] = (modes[m] || 0) + (i.approvedAmount || i.carrierAmount || 0); });
  res.json(modes);
});
app.get('/api/dashboard/monthly-spend', (req, res) => {
  const invoices = readData('invoices.json');
  const months = {};
  invoices.forEach(i => { const d = i.shipDate || i.createdAt || ''; const m = d.substring(0, 7); if (m) months[m] = (months[m] || 0) + (i.approvedAmount || i.carrierAmount || 0); });
  const sorted = Object.entries(months).sort(([a], [b]) => a.localeCompare(b));
  res.json(sorted.map(([month, amount]) => ({ month, amount })));
});

// SHIPPERS
app.get('/api/shippers', (req, res) => { res.json(readData('shippers.json')); });
...
app.get('/api/payments/upcoming', (req, res) => {
  ...
  const byShipper = {};
  paid.forEach(i => { byShipper[i.shipper] = (byShipper[i.shipper] || 0) + (i.approvedAmount || 0); });
  const topShippersPaid = Object.entries(byShipper).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, amount]) => ({ name, amount }));
  const byShipperCount = {};
  paid.forEach(i => { byShipperCount[i.shipper] = (byShipperCount[i.shipper] || 0) + 1; });
  const topShippersVolume = Object.entries(byShipperCount).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([name, count]) => ({ name, count }));
  const byDate = {};
  paid.forEach(i => { byDate[i.payDate] = (byDate[i.payDate] || 0) + (i.approvedAmount || 0); });
  const dateData = Object.entries(byDate).sort(([a], [b]) => a.localeCompare(b)).map(([date, amount]) => ({ date, amount, cumulative: 0 }));
  let cum = 0;
...
app.get('/api/uploads/history', (req, res) => {
  ...
  res.json({
    ...
    files: uploads.map(d => ({ id: d.id, fileName: d.originalName || d.name, fileSize: d.size ? Math.round(d.size / 1024) : 0, barcode: d.barcode || '', uploadDate: d.uploadedAt, path: d.path }))
  });
});
app.post('/api/uploads/invoice', upload.single('file'), (req, res) => {
  ...
  const docs = readData('documents.json');
  const doc = {
    id: nextId(docs), invoiceId: req.body.invoiceId ? parseInt(req.body.invoiceId) : null,
    docType: 'invoice', account: req.body.account || '210800 AHLSTROM MOVERS',
    name: req.body.name || req.file.originalname, originalName: req.file.originalname,
    filename: req.file.filename, path: `/uploads/${req.query.type || 'general'}/${req.file.filename}`,
    size: req.file.size, mimeType: req.file.mimetype, barcode: req.body.barcode || '',
    uploadedBy: req.body.uploadedBy || 'FRANKLIN RODRIGUEZ', uploadedAt: new Date().toISOString(), notes: req.body.notes || ''
  };
  docs.unshift(doc);
  writeData('documents.json', docs);
  res.status(201).json(doc);
